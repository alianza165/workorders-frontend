...
export default function EquipmentTypeAnalysis() {
  ...
  const fetchDepartments = useCallback(async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8001'}/backend/api/departments/`, 
        {
          headers: {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8001'}/backend/api/analytics/equipment-types/?${params.toString()}`, 
        {
          headers: {
            'Authorization': `Token ${token}`,
          },
