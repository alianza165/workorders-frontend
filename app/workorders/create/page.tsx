...
export default function CreateWorkOrder() {
  ...
  const fetchAllEquipment = useCallback(async (search = '') => {
    try {
      let url = `${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8001'}/backend/api/equipment/`;
      if (search) {
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      try {
        ...
        const [partsRes, workTypesRes] = await Promise.all([
          fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8001'}/backend/api/parts/`, {
            headers: {
              'Authorization': `Token ${token}`,
              'Content-Type': 'application/json',
            },
          }),
          fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8001'}/backend/api/work-types/`, {
            headers: {
              'Authorization': `Token ${token}`,
              'Content-Type': 'application/json',
            },
    ...
    fetchInitialData();
  }, [token, router, fetchAllEquipment]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8001'}/backend/api/workorders/`, {
        method: 'POST',
        headers: {
          'Authorization': `Token ${token}`,
          'Content-Type': 'application/json',
