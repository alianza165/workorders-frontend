...
export default function WorkOrderDetail() {
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      try {
        ...
        const workTypesRes = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/work-types/`, {
          headers: {
        ...
        const [orderRes, historyRes] = await Promise.all([
          fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/workorders/${id}/`, {
            headers: {
              'Authorization': `Token ${token}`,
              'Content-Type': 'application/json',
            },
          }),
          fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/workorders/${id}/history/`, {
            headers: {
              'Authorization': `Token ${token}`,
              'Content-Type': 'application/json',
            },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const historyRes = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/workorders/${id}/history/`, {
        headers: {
          'Authorization': `Token ${token}`,
          'Content-Type': 'application/json',
        },
  ...
  const handleAction = async (action: string, formData: { assigned_to: string; target_date: string; remarks: string } | null = null) => {
    ...
    try {
      ...
      const endpoint = action === 'accept' 
        ? `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/workorders/${workOrder.id}/accept/`
        : `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/workorders/${workOrder.id}/${action}/`;
  ...
  const handleCloseWorkOrder = async (closed: boolean, remarks: string) => {
    try {
      ...
      // First close the work order
        ...
        const closeResponse = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/workorders/${workOrder.id}/close/`, {
        method: 'POST',
        headers: {
          'Authorization': `Token ${token}`,
          'Content-Type': 'application/json',
